...
import './OtherTaskDetail.css';
import { useNavigate } from 'react-router-dom';


const OtherTaskDetail = ({ task }) => {
  ...
  const handleEditClick = () => {
    navigate('/edit-task'); // Chuyển hướng tới trang chỉnh sửa
  };

  return (
    <div className="other-task-detail">
      ...
      <div className="task-pane">
        ...
        {/* Nút Chỉnh sửa và checkbox */}
        <div className="task-footer">
          <div className="task-checkbox">
            <input type="checkbox" id="complete-task" />
            <label htmlFor="complete-task"> Đánh dấu hoàn thành</label>
          </div>
          <button className="edit-task-button" onClick={handleEditClick}>Chỉnh sửa</button>
        </div>
      </div>
    </div>
  );
...
export default OtherTaskDetail;