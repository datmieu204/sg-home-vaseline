...
import '../TaskEditing/TaskEditing.css';

const TaskCreating = ({ role }) => { // Nhận role từ props
  const [taskName, setTaskName] = useState('');
  const [selectedDepartment, setSelectedDepartment] = useState('');
  const [startDate, setStartDate] = useState(''); // Mặc định là rỗng
  const [startTime, setStartTime] = useState(''); // Mặc định là rỗng
  const [endDate, setEndDate] = useState(''); // Mặc định là rỗng
  const [endTime, setEndTime] = useState(''); // Mặc định là rỗng
  ...
  return (
    <>
      <h1 className="page-title">Thêm nhiệm vụ</h1>
